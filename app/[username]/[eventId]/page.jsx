...
import { isUserGoogleConnected } from "@/actions/users";
...
export default async function EventBookingPage({ params }) {
  ...
  const organiserGoogleConnected = await isUserGoogleConnected(event?.user?.clerkUserId);
  ...
  return (
    <div className="flex flex-col justify-center lg:flex-row px-4 py-8">
      ...
      <Suspense fallback={<div>Loading booking form...</div>}>
        <BookingForm event={event} availability={availability} organiserGoogleConnected={organiserGoogleConnected} />
      </Suspense>
    </div>
  );
}
